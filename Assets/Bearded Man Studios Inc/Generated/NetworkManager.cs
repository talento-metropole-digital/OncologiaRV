...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] basicCube_sample1NetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is basicCube_sample1NetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (basicCube_sample1NetworkObject.Length > 0 && basicCube_sample1NetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(basicCube_sample1NetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<basicCube_sample1Behavior>();
						}
					}
		...
		[Obsolete("Use InstantiatebasicCube_sample1 instead, its shorter and easier to type out ;)")]
		public basicCube_sample1Behavior InstantiatebasicCube_sample1NetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(basicCube_sample1NetworkObject[index]);
			var netBehavior = go.GetComponent<basicCube_sample1Behavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<basicCube_sample1Behavior>().networkObject = (basicCube_sample1NetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of basicCube_sample1
		/// </summary>
		/// <returns>
		/// A local instance of basicCube_sample1Behavior
		/// </returns>
		/// <param name="index">The index of the basicCube_sample1 prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public basicCube_sample1Behavior InstantiatebasicCube_sample1(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(basicCube_sample1NetworkObject[index]);
			var netBehavior = go.GetComponent<basicCube_sample1Behavior>();
